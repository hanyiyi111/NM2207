...
\textbf{Solution:} ``csv'' stands for Comma-Seperated Values files. CSVs
are popular because they are lightweight and compact, which means they
take up less space and can be transferred faster than Excel files.
Moreover, CSV files are plain text, which means they are less prone to
corruption and can be easily inspected and modified. \textbf{Question
1.2:} load the \texttt{tidyverse} package to work with \texttt{.csv}
files in R.
...
\begin{figure}
\includegraphics[width=800px,height=400px]{Screenshot} \caption{This is a screenshot.}\label{fig:unnamed-chunk-5}
\end{figure}

\textbf{Question 1.6:} What does the \texttt{skip} argument in the
read\_csv() function do?

\textbf{Solution:} We want to skip rows that have missing values while
reading the file. We can use the na\_values parameter of the read\_csv()
function to specify the values that should be considered missing values
and then use the skip parameter to skip the rows with missing values.
...
\NormalTok{insert\_name\_of\_variable }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"sales\_data.csv"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{aes}\NormalTok{(}\AttributeTok{x=}\NormalTok{age,}\AttributeTok{y=}\NormalTok{ratings)}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{ggplot}\NormalTok{(playlist\_data, }\FunctionTok{aes}\NormalTok{(Age, Rating)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{shape =} \DecValTok{2}\NormalTok{, }\AttributeTok{colour =} \StringTok{"red"}\NormalTok{, }\AttributeTok{size =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{?}\FunctionTok{geom\_point}\NormalTok{( ) }\CommentTok{\# \textless{}{-}{-} Hint: Use ? to learn more about geom\_point and use appropriate values for shape}
\end{Highlighting}
\end{Shaded}
