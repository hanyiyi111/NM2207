...
**Solution:** "csv" stands for Comma-Seperated Values files. CSVs are popular because they are lightweight and compact, which means they take up less space and can be transferred faster than Excel files. Moreover, CSV files are plain text, which means they are less prone to corruption and can be easily inspected and modified.
<br>
<br>
...
```
<br>
```{r, out.height= "400px",out.width= "800px",echo=TRUE,eval=TRUE,fig.cap="This is a screenshot."}
...
**Solution:** We want to skip rows that have missing values while reading the file. We can use the na_values parameter of the read_csv() function to specify the values that should be considered missing values and then use the skip parameter to skip the rows with missing values. 
<br>
<br>
...
ggplot(playlist_data, aes(Age, Rating)) +
  geom_point(shape = 2, colour = "red", size = 2)
